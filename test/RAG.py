    ...
    with st.sidebar:
        ...
        with tab2:
            ...
            with col_add1:
                ...
                new_category = st.selectbox("分类", ["政策法规", "历史事件", "领土主权", "政治理论", "社会治理", "其他"], index=0)
            with col_add2:
                new_violation_type = st.selectbox("违规类型", ["无", "涉台", "涉港", "涉疆", "涉藏", "涉军", "涉领导人", "侮辱英烈","历史虚无主义","分裂国家", "其他违规"], index=0)

            col_add3, col_add4 = st.columns(2)
            with col_add3:
                new_severity = st.selectbox("严重程度", ["无", "轻微", "中等", "严重"], index=0)
            with col_add4:
                new_sources = st.text_input("来源", placeholder="请输入参考来源")

            new_suggestion = st.text_input("修改建议", placeholder="请输入修改建议")
            ...
            if docs_for_edit:
                ...
                with st.form("edit_doc_form"):
                    ...
                    with col_e3:
                        edit_violation_type = st.text_input("违规类型", value=str(current.get('violation_type', '无')))
                    with col_e4:
                        severity_options = ["无", "轻微", "中等", "严重"]
                    ...
                    with col_e5:
                        edit_suggestion = st.text_input("修改建议", value=str(current.get('suggestion', '无')))
                    with col_e6:
