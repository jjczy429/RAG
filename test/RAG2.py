...
class Config:
    ...
    # 方案B：固定绑定并写回到项目内的 CSV
    CSV_KNOWLEDGE_BASE_PATH = "political_knowledge_base.csv"
...
class VectorDatabase:
    ...
    def bind_csv(self, csv_path: str, text_column: str, category_column: Optional[str] = None, other_columns: Optional[List[str]] = None) -> None:
    ...
    def build_from_csv(self, csv_path, text_column, category_column=None, bind_to_csv_path: Optional[str] = None):
        ...
        if bind_to_csv_path:
            ...
            self.bind_csv(bind_to_csv_path, text_column, category_column, other_columns=other_cols)
...
def main():
    ...
    with st.sidebar:
        ...
        with tab1:
            st.subheader("从CSV构建")
            st.caption(f"固定知识库文件：{Config.CSV_KNOWLEDGE_BASE_PATH}")

            if not os.path.exists(Config.CSV_KNOWLEDGE_BASE_PATH):
                st.warning("⚠️ 固定CSV文件不存在，请先在项目目录放置该文件。")
                df = None
            else:
                df = pd.read_csv(Config.CSV_KNOWLEDGE_BASE_PATH)
            ...
            if df is not None:
                ...
                if st.button("构建/刷新向量数据库", type="primary"):
                    with st.spinner("构建中..."):
                        # 使用固定的CSV文件路径
                        csv_path = Config.CSV_KNOWLEDGE_BASE_PATH
                        if not os.path.exists(csv_path):
                            ...
                            count = st.session_state.vector_db.build_from_csv(
                                csv_path, text_col, cat_col, bind_to_csv_path=csv_path
                            )
                            st.success(f"✅ 成功从 {os.path.basename(csv_path)} 导入 {count} 条数据")
                            st.info("知识库已绑定该文件，后续增删改将自动同步。")
                            st.rerun()
